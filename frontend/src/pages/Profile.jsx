...
export default function Profile() {
      ...
      try {
        ...
        const badgesSnap = await getDocs(
          query(
            ...
            where(documentId(), "in", keys.slice(0, 30))
          )
        );

        setEarnedBadges(
          badgesSnap.docs.map((d) => ({
            id: d.id,
            ...d.data(),
          }))
        );
  ...
  async function handlePhotoChange(e) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      setError("Tumia picha ya JPG, PNG au WEBP.");
      return;
    }

    if (file.size > MAX_SIZE) {
      setError("Picha isizidi 3MB.");
      return;
    }
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="card flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-ivory-muted text-sm font-mono">
              {profile.regNumber}
            </p>
        ...
        <div className="card">
          <p className="label mb-3">
            Beji Zangu ({earnedBadges.length})
          </p>
          ...
          {earnedBadges.length > 0 ? (
            <div className="flex flex-wrap gap-3">
              {earnedBadges.map((b) => (
                ...
                >
                  ...
                  <span className="text-xs text-center text-ivory-muted mt-1">
                    {b.name}
                  </span>
                </div>
              ))}
            ...
            <p className="text-ivory-muted text-sm">
              Bado hujapata beji. Endelea kujifunza!
            </p>
          )}
        </div>
